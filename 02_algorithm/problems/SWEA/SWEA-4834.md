...
def counting_sort(input_arr, k):
    ...
    max_count = max(counting_arr)
    max_card = max(idx for idx, count in enumerate(counting_arr) if count == max(counting_arr))
...
for test_case in range(1, 1 + TC):
    ...
    card, count = counting_sort(cards, 9)

    print(f'#{test_case} {card} {count}')

```
- enumerate 함수를 드디어 실전에서 사용했습니다. 인덱스와 값을 어떻게 매치해서 뽑아낼까 한참을 고민했지만 제 스스로 생각해내진 못하고 AI의 도움을 받았습니다. 역시 지식으로 알고 있는 것과 실제 사용하는 건 별개라는 생각을 했습니다.

- 당연히 모듈을 사용한 것보다 코드가 길기는 하지만 몇 배는 더 마음에 드는 코드입니다.
