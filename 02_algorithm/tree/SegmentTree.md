...
## 목차
- [세그먼트 트리?](#세그먼트-트리)
    - [iterative 세그트리](#iterative-세그트리)
    - [recur 세그트리](#recur-세그트리)
- [연습문제](#연습문제)
...
### 1. Iterative 세그트리  
반복문 기반 세그먼트 트리로 지금까지 내가 구현했던 세그먼트 트리를 칭하는 말이다.  
처음 세그트리를 만들 때 `seg = [0] * (4*N)` 으로 구현하면 재귀 세그트리, `seg = [0] * (2 * size)`로 구현하면 반복 세그트리이다. ( 주로 그렇다. )
  

### 2. Recur 세그트리
재귀 기반 세그트리이다. 영욱님이 처음 설명해주신, 유정님의 블로그에 써있는 방식의 세그트리가 이것이다. GPT의 표현으로는 `재귀적으로 구간을 분할하는 트리를 넉넉한 배열에 저장하는 구현` 이라고 한다.  
`반복 세그트리`와의 차이는 다음과 같다.
    - 반복 세그트리는 노드에 값을 저장하지만 재귀 세그트리는 노드에 구간 정보를 저장한다.
        - 루트 노드에는 전체 구간이 기록되고 그 자식 노드에는 왼쪽 반, 오른쪽 반이 나눠서 저장된다. 이게 리프 노드까지 반복되어 리프 노드는 하나의 값을 가지게 된다.

  
### 3. 차이 정리
1. Iterative Segment Tree:  
    노드에 `값`이 저장되고 리프가 모두 같은 깊이에 있다.
    - 잘 맞는 문제:
        - 점 업데이트
        - 구간 합, 구간 최소/ 최대

2. Recur Segment Tree:  
    노드에 `구간정보`가 저장되고 리프의 깊이가 다 다를 수 있다.
    - 잘 맞는 문제:
        - 구간 업데이트
        - Lazy propagation
        - 복잡한 구간 정보
